    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        if "device_map" in tokenizer_config and not isinstance(tokenizer_config["device_map"], str):
            ...
            str_type_device = str(tokenizer_device)
            raise TypeError(f"'device_map' is not in str type, got {type(tokenizer_device)}, change to {str(tokenizer_device)}. Please set the value in str format (e.g. 'auto') or do not pass it into tokenizer (for TPU).")
